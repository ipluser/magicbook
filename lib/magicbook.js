;(function (global, $) {  // eslint-disable-line
  ...
  var toStringEqual = Magicbook.toStringEqual = function toStringEqual(obj, toStr) {
    return objectToString.call(obj) === toStr;
  };

  var isFunction = Magicbook.isFunction = function isFunction(fn) {
    return toStringEqual(fn, '[object Function]');
  };

  var isString = Magicbook.isString = function isString(s) {
    return toStringEqual(s, '[object String]');
  };

  var isArray = Magicbook.isArray = function isArray(arr) {
    return toStringEqual(arr, '[object Array]');
  };

  Magicbook.isNumber = function isNumber(obj) {
    return toStringEqual(obj, '[object Number]');
  };

  var isUndefined = Magicbook.isUndefined = function isUndefined(obj) {
    return typeof obj === 'undefined';
  };

  var parseArray = Magicbook.parseArray = function parseArray(obj) {
  ...
  var getPropertyByDefaultValue = Magicbook.getPropertyByDefaultValue = function getPropertyByDefaultValue(obj, name, defaultValue) {
    return obj && name && obj[name] || defaultValue;
  };

  var hasOwnProperty = Magicbook.hasOwnProperty = function hasOwnProperty(obj, key) {
    return {}.hasOwnProperty.call(obj, key);
  };

  var requestGet = Magicbook.requestGet = function requestGet(url, callback) {
    if (!url) {
      return;
    }
  ...
  var addClassName = Magicbook.addClassName = function addClassName(selector, className) {
