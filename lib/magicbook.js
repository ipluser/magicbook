...
(function(global, factory) {
  ...
  function randomString() {
    return globalMath.random().toString(36).substr(2);
  }
  ...
  var Magicbook = global.Magicbook = function Magicbook(cfg) {
    ...
    function initLayout() {
      ...
      if (!navigatorSelector && !contentSelector) {
        ...
        if (!$container.length) {
          ...
          $(globalDocument.body).prepend($container);
        }
    ...
    function initNavigator() {
      ...
      executeCallbackQueue(defaultCallbackQueue, 'prepare', self);
  ...
  proto.render = function render(url, callback) {
    ...
    executeCallbackQueue(defaultCallbackQueue, 'prepare', self);
    executeCallbackQueue(extraCallbackQueue, 'prepare', self);
  ...
  proto.moveTo = function moveTo(options) {
    ...
    (isBodyScroller() && $(globalDocument.body) || self.$contentWrap).stop().animate({
      scrollTop: scrollTop
    }, _options.duration);
  };
  ...
  Magicbook.randomString = randomString;

  if ((typeof module === 'object' && typeof module.exports === 'object')
      || (typeof define === 'function' && define.amd)) {  // eslint-disable-line
    return Magicbook;
  }
}));
